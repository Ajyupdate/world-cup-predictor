...
class EloRatingSystem:
    ...
    def update_match(self, home_team, away_team, home_goals, away_goals, home_advantage=0, importance=1):
        ...
        adjusted_home_rating = (
            self.ratings[home_team] + (75 if home_advantage else 0)
        )
        expected_home = self.expected_score(adjusted_home_rating, away_rating)
        expected_away = self.expected_score(away_rating, adjusted_home_rating)
